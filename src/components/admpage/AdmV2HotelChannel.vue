  ...
  <div class="">
    ...
    <div class="container">
      ...
      <el-dialog title="类别排序" :visible.sync="sortTypeDialogFormVisible">
        ...
        <el-transfer filterable filter-placeholder="请输入频道" v-model="NewlistsortTypeData" :data="listsortTypeData"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      sortType() {
         // 左边的数组
        let copylistsortTypeData=this.listsortTypeData
        // 右边的数组id
        let copyNewlistsortTypeData=[];
        for(var i=0;i<this.NewlistsortTypeData.length;i++){
          copyNewlistsortTypeData.push({
            "id":this.NewlistsortTypeData[i]
          })
          // 筛选出后的左边数组
            copylistsortTypeData =copylistsortTypeData.filter(item => item.id !== this.NewlistsortTypeData[i])
        }
       
        // // 所有请求的参数id
        let arr7 = [...copyNewlistsortTypeData, ...copylistsortTypeData]
        this.$axios
          .post("/api/hoV2InChannelType/sortType", arr7)
          .then(res => {
            console.log(res.data);
      ...
      sortChannel() {
         // 左边的数组
        let copylistsortChannelData=this.listsortChannelData
        // 右边的数组id
        let copyNewlistsortChannelData=[];
        for(var i=0;i<this.NewlistsortChannelData.length;i++){
          copyNewlistsortChannelData.push({
            "id":this.NewlistsortChannelData[i]
          })
          // 筛选出后的左边数组
            copylistsortChannelData =copylistsortChannelData.filter(item => item.id !== this.NewlistsortChannelData[i])
        }
       
        // // 所有请求的参数id
        let arr7 = [...copyNewlistsortChannelData, ...copylistsortChannelData]
        this.$axios
          .post("/api/hoV2InChannelName/sortChannel", arr7)
          .then(res => {
            console.log(res.data);
